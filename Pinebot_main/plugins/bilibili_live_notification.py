...
unaddable_group = [176763307] # 通知 但不可操作群 本人可操作
...
async def handle_group_message(ctx):
	...
	if g not in unactivate_group:
		...
		if args[0] == u"-liveadd" and len(args) == 2:
			if g in unaddable_group:
				if ctx["sender"]["user_id"] != 384065633:
					await bot.send_group_msg(group_id = g, message = u"没有在本群添加的权限,请联系管理员")
					return
			try:
				room_id = int(args[1])
			except:
